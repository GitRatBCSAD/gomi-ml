#!/usr/bin/env python3
"""
Extract Lizard structural complexity metrics for every JIT-Fine commit.

Input:  dataset/jitfine_{project}_recovered.csv
        (from scripts/recover_jitfine_messages.py — commit_hash, project,
         message, buggy, author_date_unix_timestamp, + raw Kamei fields)

21 projects, one repo each (all verified apache/<name> GitHub URLs — see
REPO_MAP). Full clones (no --filter=blob:none), same convention as
extract_deepjit_lizard.py / extract_apache_lizard.py / extract_geocabral_lizard.py
— shared dataset/.clones/ dir, no lazy network fetches during Lizard.
...
For each commit:
  1. List changed source files (git diff-tree --cc --diff-filter=AM — the
     ...
     plain diff-tree for non-merges, so unconditionally safe)
...
Output: dataset/jitfine_{project}_lizard.csv
Columns: commit_id, project, avg_ccn, avg_nloc, func_count, avg_params, file_count, buggy

Usage:
    python scripts/extract_jitfine_lizard.py                  # all 21 projects
    python scripts/extract_jitfine_lizard.py --project giraph # single project
    python scripts/extract_jitfine_lizard.py --verify         # sample only, no CSV written
    python scripts/extract_jitfine_lizard.py --workers 16
"""
...
# All 21 verified via `git ls-remote` before writing this (see chat log) —
# no "fabric vs Fabric8"-style guessing here.
REPO_MAP = {
...
def load_recovered_csv(project: str) -> list:
    path = DATASET_DIR / f"jitfine_{project}_recovered.csv"
    if not path.exists():
        print(f"  [{project}] missing {path} — run recover_jitfine_messages.py first")
        return []
...
def process_project(project: str, workers: int, verify_only: bool) -> Path | None:
    ...
    out_path = DATASET_DIR / f"jitfine_{project}_lizard.csv"
    done_ids = set()
    if out_path.exists():
...
def main():
    ...
    projects = [single] if single else list(REPO_MAP.keys())
    ...
    print()
    print("Next steps:")
    print("  1. Upload jitfine_*_recovered.csv + jitfine_*_lizard.csv to HuggingFace dataset repo.")
    print("  2. Add a notebook cell (modeled on cell 19/20 ApacheJIT loader) that:")
    print("     - loads jitfine_{project}_recovered.csv + jitfine_{project}_lizard.csv")
    ...
    print("     - appends to the training pool alongside DeepJIT + ApacheJIT + geocabral")
