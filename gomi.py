...
#
# Layer 3: Logistic Regression (train via scripts/train_risk_model.py, load at runtime)
...
# Layer 3 — pre-trained Logistic Regression (produced by scripts/train_risk_model.py)
...
HF_RISK_MODEL_REVISION = os.getenv("GOMI_RISK_MODEL_REVISION")
...
def load_sentiment_model():
    ...
    source_desc = model_source if not revision else f"{model_source}@{revision}"
    print(f"  Loaded DistilBERT sentiment model from: {source_desc}")
    return classifier
...
def _load_deepjit_records(sentiment_clf) -> list[dict]:
    ...
    for pkl_path in DEEPJIT_PKLS:
        ...
        resolved_pkl = _resolve_dataset_file(filename)
        if not resolved_pkl or not os.path.isfile(resolved_pkl):
            project = filename.replace("_test_raw.pkl", "")
            source_hint = pkl_path
            if HF_DATASET_REPO:
                source_hint = f"{pkl_path} (or {HF_DATASET_REPO})"
            print(f"    [skip] DeepJIT project '{project}' not found: {source_hint}")
            continue
        ...
        feature_name = filename.replace("_test_raw.pkl", "_k_feature.csv")
        feature_csv = _resolve_dataset_file(feature_name)
        ent_by_hash: dict[str, float] = {}
        if feature_csv and os.path.isfile(feature_csv):
...
def _load_apachejit_records(sentiment_clf) -> tuple[list[dict], list[dict]]:
    ...
    """
    resolved_csv = _resolve_dataset_file(os.path.basename(APACHEJIT_CSV))
    if not resolved_csv or not os.path.isfile(resolved_csv):
        source_hint = APACHEJIT_CSV
        if HF_DATASET_REPO:
            source_hint = f"{APACHEJIT_CSV} (or {HF_DATASET_REPO})"
        print(f"    [skip] ApacheJIT CSV not found: {source_hint}")
        return [], []
...
def load_risk_model() -> tuple:
    """
    Loads the pre-trained Logistic Regression risk fusion model.

    The model must be trained offline via scripts/train_risk_model.py,
    which fits on DeepJIT + ApacheJIT and saves artifacts to DATASET_DIR.
    ...
    if HF_RISK_MODEL_REPO:
        ...
        model_path = hf_hub_download(
            ...
            filename=os.path.basename(RISK_MODEL_PATH),
            repo_type="model",
        ...
        shap_path = hf_hub_download(
            repo_id=HF_RISK_MODEL_REPO,
            filename=os.path.basename(RISK_SHAP_BACKGROUND_PATH),
            repo_type="model",
            revision=HF_RISK_MODEL_REVISION,
            token=_get_hf_token(),
            force_download=True,  # ALWAYS override local cache
